...
//import * as watchify from 'browserify';
...
export class Builder {
	...
	buildClassFn(relPath: string): string {
		...
		fs.writeFileSync($path(this.destPath, relPath, './class.ts'), classCode);	
		return classCode;		 
	};

};

function transformTpl(code: string, dd: DataDispatcher): string{
...
function transformFgDir(nameSpace: string, srcPath: string, destPath: string): IComponentSet {	
    ...
    function iterate(name: string, objPath: string[], srcPath: string, destPath: string): IFgMeta {
        ...
        if (serverUtils.fileExist(tplPath)){
			...
			const dd = new DataDispatcher;			
            const tplDataCode = transformTpl(meta.tpl, dd);
			const dtCode = 'export default ' + dd.genTransformer();
            fs.writeFileSync($path(destPath, './dataTransform.ts'), dtCode);			 
            const tplDestPath = $path(destPath, './tpl.ts');
            fs.writeFileSync(tplDestPath, tplDataCode);
        }else{
            meta.tpl = null;
        };
        const classPath = $path(srcPath, './class.ts');
		if (serverUtils.fileExist(classPath)){
            meta.classFn = fs.readFileSync(classPath).toString();
            fs.writeFileSync($path(destPath, './class.ts'), meta.classFn);			 
        }else{
            meta.classFn = null;
        };
