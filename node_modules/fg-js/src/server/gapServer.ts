...
import {default as gaps} from '../common/gaps';
...
export function parse(ast: IAstNode, dataDispatcher: DataDispatcher, parents: IGapData[], html: string): IGapData{
	...
	for (let gapItem of gapList){
		const meta = gapItem.gap.parse(ast, dataDispatcher, parents, html);
		if (meta){
			return meta;
		};
	};
