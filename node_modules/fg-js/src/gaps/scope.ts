		...
		if (newLen !== oldLen){
			if (newLen > oldLen){
				if (newData.items[newLen - 1] === oldData.items[oldLen - 1]){
					start = newLen - oldLen;
					const newContent = this.renderParts(context, newData.items.slice(0, start), meta);
					const anchor = anchorMgr.find(context, this, 'up');
					anchorMgr.insertHTML(anchor, 'after', newContent);
				} else {
					end = oldLen;
